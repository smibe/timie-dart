...
Future<Null> saveUsage() async {
  if (UsageStore.lastWriteTime != null &&
      UsageStore.lastWriteTime.millisecondsSinceEpoch >=
          Usage.Usage.getStartOfDay(-1).millisecondsSinceEpoch) {
  ...
  UsageStore.store(
      Usage.Usage.getStartOfDay(-1), timeYesterday, usageStatsYesterday);
...
Future<Null> main() async {
  ...
  silentLogIn().then((u) async {
    ...
    var result = await AndroidAlarmManager.periodic(
        const Duration(minutes: 30), alarmID, saveUsage);
    print('$result');
  });
}
...
class HomeState extends State<Home> {
  ...
  Widget build(BuildContext context) {
    if (this.loggedIn)
      return new TimieHomePage.TimieHomePage(
        title: "Timie",
      );
